...
void    *mlx_new_image(mlx_ptr_t *mlx_ptr, int width, int heigth)
{
  ...
  newimg->vertexes[4] = width;  newimg->vertexes[5] = -heigth;
  newimg->vertexes[6] = 0.0;  newimg->vertexes[7] = -heigth;
  newimg->buffer = malloc(UNIQ_BPP*width*heigth);
  bzero(newimg->buffer, UNIQ_BPP*width*heigth);
...
mlx_img_ctx_t	*add_img_to_ctx(mlx_img_list_t *img, mlx_win_list_t *win)
{
  ...
  glTexImage2D(
	       ...
	       img->width, img->heigth, 0,           /* width, heigth, border */
...
void    mlx_put_image_to_window(mlx_ptr_t *mlx_ptr, mlx_win_list_t *win_ptr, mlx_img_list_t *img_ptr, int x, int y)
{
  ...
  // update texture
  glBindTexture(GL_TEXTURE_2D, imgctx->texture);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, img_ptr->width, img_ptr->heigth, 0,
